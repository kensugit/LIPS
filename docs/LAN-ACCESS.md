...
2026-09-20修正: 初版の電源エイリアス誤記を `STANDBYIDLE` / `HIBERNATEIDLE` に修正。Windows PowerShell 5.1で実際の読取とAC/DC値の識別を検証。電源読取を資格情報入力・フォルダー作成より前に移動。初版が電源読取で停止した状態（旧タスクXMLのみ、現タスクと一致、ゲートウェイタスク・ポート・Firewallなし）はバックアップを維持して再開できる。他の部分適用状態は停止する。
