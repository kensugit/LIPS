...
(async()=>{
 ...
 const bundles=fs.readdirSync('catalogs').filter(n=>n.startsWith('アルカン.')&&n.endsWith('.catalog.json')).map(n=>JSON.parse(fs.readFileSync('catalogs/'+n,'utf8')));
 ...
 try {
  ...
  const query='/api/search?text='+encodeURIComponent('アルカン');
  const result=await get(query);assert.equal(result.rows.length,164);assert.equal(result.capped,false);
  ...
  await page.locator('#text').fill('アルカン');await page.locator('button[type=submit]').click();
  await page.waitForFunction(()=>document.querySelector('#count').textContent==='164件');
  await page.screenshot({path:'artifacts/arcan-search.png',fullPage:true});
  await page.locator('#text').fill('07047221');await page.locator('button[type=submit]').click();
  ...
  assert((await page.locator('#detail-body').textContent()).includes('☆【アルカン】9月ボルドーリスト.pdf'));
  await page.locator('summary').click();
  for(const s of ['納品価格','23980','3本単位','税区分','在庫'])assert((await page.locator('details').textContent()).includes(s));
  assert.deepEqual(errors,[]);
  const output=process.env.CATALOG_URL?'artifacts/arcan-production-verification.json':'artifacts/arcan-ui-verification.json';
  fs.writeFileSync(output,JSON.stringify({base,products:164,allValuesMatched:true,sourceEvidenceVerified:true,errors},null,2));
  console.log('164 Arcan products, source evidence, unknown inventory and UI verified');
