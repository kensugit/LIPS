...
for supplier in list-german-beer list-others wine-list-austria wine-list-france wine-list-germany wine-list-italy wine-list-spain-portugal-japan; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 758 Daiei products.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-daiei-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nDaiei 758 products imported and verified.\n'
