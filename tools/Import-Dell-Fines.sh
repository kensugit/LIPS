...
[[ $image == sha256:574e372723b8989eb57f70b9faa9a75d9f22f6dc8e367a728f66a563787d57cb ]] || { echo 'Production image changed; review compatibility before applying.' >&2; exit 1; }
...
run_bridge() {
  docker run --rm --network host --read-only --cap-drop ALL --security-opt no-new-privileges \
    ...
    ' sh "/import/data/$1.catalog.json" "/import/data/$1.xls" "$2"
}
for supplier in fines; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 2090 Fines products.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-fines-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nFines 2090 products imported and verified.\n'
