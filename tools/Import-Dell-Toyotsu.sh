...
for supplier in toyotsu; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 640 Toyotsu products.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-toyotsu-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nToyotsu 640 products imported and verified.\n'
