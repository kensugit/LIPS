...
for supplier in arcan; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 164 Arcan products.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-arcan-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nArcan 164 products imported and verified.\n'
