...
sha=b4b7f593b65a4120cb08e823dce9461014de4b76d2c115406e6d45ba5b38755b
package=/home/catalogdeploy/incoming/$sha.tar
printf '%s  %s\n' "$sha" "$package" | sha256sum --check --quiet
stage=$(mktemp -d /var/tmp/lips-terravert-20260921.XXXXXX)
chmod 700 "$stage"
...
docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges \
  ...
  'test -r /import/runtime/CatalogPdfBridge.dll && test -r /import/data/terravert.catalog.json && test -r /import/data/terravert.xlsx'
