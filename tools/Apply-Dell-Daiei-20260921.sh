...
sha=f17c680e00dc1c37bf1e8a70cbc02e03fe10b0cd005411b5df057001628c12a0
package=/home/catalogdeploy/incoming/$sha.tar
printf '%s  %s\n' "$sha" "$package" | sha256sum --check --quiet
stage=$(mktemp -d /var/tmp/lips-daiei-20260921.XXXXXX)
chmod 700 "$stage"
...
docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges \
  ...
  'test -r /import/runtime/CatalogPdfBridge.dll && test -r /import/data/list-german-beer.catalog.json && test -r /import/data/list-german-beer.pdf'
