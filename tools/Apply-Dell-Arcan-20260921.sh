...
sha=c052a0cb052efc96f54b37af7334f5e5a74d3dfcea41546962c0bd2382a841c6
package=/home/catalogdeploy/incoming/$sha.tar
printf '%s  %s\n' "$sha" "$package" | sha256sum --check --quiet
stage=$(mktemp -d /var/tmp/lips-arcan-20260921.XXXXXX)
chmod 700 "$stage"
...
docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges \
  ...
  'test -r /import/runtime/CatalogPdfBridge.dll && test -r /import/data/arcan.catalog.json && test -r /import/data/arcan.pdf'
