...
sha=b76e0ce32229af56c9ee9b5e507918d361dd32f26763edf01a807ee6cf9541a7
package=/home/catalogdeploy/incoming/$sha.tar
printf '%s  %s\n' "$sha" "$package" | sha256sum --check --quiet
stage=$(mktemp -d /var/tmp/lips-nippon-liquor-20260921.XXXXXX)
chmod 700 "$stage"
...
docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges \
  ...
  'test -r /import/runtime/CatalogPdfBridge.dll && test -r /import/data/shipping.catalog.json && test -r /import/data/shipping.xlsx'
