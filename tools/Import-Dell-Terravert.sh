...
for supplier in terravert; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 1090 Terravert products.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-terravert-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nTerravert 1090 products imported and verified.\n'
