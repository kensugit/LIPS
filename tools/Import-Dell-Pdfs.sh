...
for supplier in wine-experience finesse royal; do
  run_bridge "$supplier" --check-schema
done
if [[ $mode == --check ]]; then
  echo 'Preflight passed. No database changes. Run with --apply to import 832 source rows.'
  exit 0
fi
backup="/var/lib/catalog-deploy/backups/before-pdf-$(date +%Y%m%d-%H%M%S)-$$.dump"
...
printf '\nAll three PDF sources imported and verified.\n'
